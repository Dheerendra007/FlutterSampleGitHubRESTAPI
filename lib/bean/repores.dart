...
List<RepoResponse> repoResponseFromJson(String str) => List<RepoResponse>.from(json.decode(str).map((x) => RepoResponse.fromJson(x)));

String repoResponseToJson(List<RepoResponse> data) => json.encode(List<dynamic>.from(data.map((x) => x.toJson())));
...
enum EventsUrl { HTTPS_API_GITHUB_COM_USERS_OCTO_ORG_EVENTS_PRIVACY }

final eventsUrlValues = EnumValues({
  "https://api.github.com/users/octo-org/events{/privacy}": EventsUrl.HTTPS_API_GITHUB_COM_USERS_OCTO_ORG_EVENTS_PRIVACY
});

enum FollowingUrl { HTTPS_API_GITHUB_COM_USERS_OCTO_ORG_FOLLOWING_OTHER_USER }

final followingUrlValues = EnumValues({
  "https://api.github.com/users/octo-org/following{/other_user}": FollowingUrl.HTTPS_API_GITHUB_COM_USERS_OCTO_ORG_FOLLOWING_OTHER_USER
});

enum GistsUrl { HTTPS_API_GITHUB_COM_USERS_OCTO_ORG_GISTS_GIST_ID }

final gistsUrlValues = EnumValues({
  "https://api.github.com/users/octo-org/gists{/gist_id}": GistsUrl.HTTPS_API_GITHUB_COM_USERS_OCTO_ORG_GISTS_GIST_ID
});

enum Login { OCTO_ORG }

final loginValues = EnumValues({
  "octo-org": Login.OCTO_ORG
});

enum NodeId { MD_EY_OK9_Y_Z2_FUA_XPHD_GLVBJ_Y4_MTE2_NZ_I }

final nodeIdValues = EnumValues({
  "MDEyOk9yZ2FuaXphdGlvbjY4MTE2NzI=": NodeId.MD_EY_OK9_Y_Z2_FUA_XPHD_GLVBJ_Y4_MTE2_NZ_I
});

enum StarredUrl { HTTPS_API_GITHUB_COM_USERS_OCTO_ORG_STARRED_OWNER_REPO }

final starredUrlValues = EnumValues({
  "https://api.github.com/users/octo-org/starred{/owner}{/repo}": StarredUrl.HTTPS_API_GITHUB_COM_USERS_OCTO_ORG_STARRED_OWNER_REPO
});
